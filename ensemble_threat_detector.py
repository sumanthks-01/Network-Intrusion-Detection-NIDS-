...
class EnsembleNetworkThreatDetector:
    def __init__(self, models_config, scaler_path, encoder_path, log_file='ensemble_threat_log.txt', interface=None):
        # Load ML components
        self.models = {}
        self.model_weights = {}
        ...
        self.logger.info(f"Ensemble Threat Detector initialized with {len(self.models)} models")
    ...
    def _ensemble_predict(self, features):
        ...
        ensemble_prob = np.zeros(len(list(probabilities.values())[0]))
        total_weight = 0
        ...
        return ensemble_prediction, ensemble_prob, predictions
    ...
    def _analyze_flow(self, flow_key, flow_data):
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                ensemble_pred, ensemble_prob, individual_preds = self._ensemble_predict(features)
            
            if ensemble_pred is None:
                return
            
            threat_type = self.encoder.inverse_transform([ensemble_pred])[0]
            ...
            if threat_type != 'BENIGN':
                ...
                pred_details = " | ".join([f"{name}: {self.encoder.inverse_transform([pred])[0]}" 
    ...
    def start_monitoring(self):
        self.logger.info("Starting ensemble network monitoring...")
        try:
...
def main():
    parser = argparse.ArgumentParser(description='Ensemble Network Threat Detector')
    ...
    parser.add_argument('--log', default='ensemble_threat_detection.log', help='Log file path')
    ...
    # Configure ensemble models with weights based on performance
    models_config = {
        'XGBoost': {
            'path': 'models/xgboost_model_20250827_200402.pkl',
            'weight': 0.35  # Highest weight for best performer
        },
        'Random Forest': {
            'path': 'models/random_forest_model_20250827_200402.pkl',
            'weight': 0.30  # Second highest
        },
        'Decision Tree': {
            'path': 'models/decision_tree_model_20250827_200402.pkl',
            'weight': 0.20
        },
        'Neural Network': {
            'path': 'models/neural_network_model_20250827_200402.pkl',
            'weight': 0.15  # Lower weight due to scaling requirement
        }
    }
    
    detector = EnsembleNetworkThreatDetector(
